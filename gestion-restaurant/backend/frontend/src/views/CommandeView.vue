...
import { commandeApi, menuApi, reservationApi, tableApi } from '../services/api.js';
...
const tables = ref([]); // toutes les tables, pour le cas "nouveau client sans réservation"
...
function selectionnerReservation() {
  ...
  if (idreserv === 'nouveau') {
    // Cas "nouveau client" : on vide nomcli et idtable pour que l'utilisateur les saisisse
    formulaire.value.nomcli = '';
    formulaire.value.idtable = '';
    return;
  }

  // Cas réservation existante : nomcli et idtable déduits automatiquement
  ...
  <div class="commande-view">
    ...
    <ModalFormulaire :ouvert="modalOuverte" titre="Nouvelle commande" @fermer="fermerModal">
      ...
      <form class="formulaire" @submit.prevent="creerCommande">
        ...
        <!-- Sur table : choisir un client ayant réservé CE JOUR-LÀ,
             ou saisir un nouveau client sans réservation. -->
        <div v-if="formulaire.typecom === 'table'" class="bloc-client">
          <select v-model="formulaire.idreservSelectionnee" @change="selectionnerReservation" required>
            <option value="" disabled>Choisir un client...</option>
            <!-- Option "nouveau client" en premier, visuellement séparée -->
            <option value="nouveau">+ Nouveau client (sans réservation)</option>
            <option v-if="reservationsDuJour.length > 0" disabled>── Clients ayant réservé ──</option>
            <option v-for="r in reservationsDuJour" :key="r.idreserv" :value="r.idreserv">
              {{ r.nomcli }} (table {{ r.idtable }})
            </option>
          </select>

          <!-- Champs libres si "nouveau client" est sélectionné -->
          <template v-if="formulaire.idreservSelectionnee === 'nouveau'">
            <input
              v-model="formulaire.nomcli"
              type="text"
            ...
            <select v-model="formulaire.idtable" required>
              ...
              <option v-for="t in tables" :key="t.idtable" :value="t.idtable">
                {{ t.idtable }} - {{ t.designation }}
                {{ t.occupation == 1 ? '(occupée)' : '' }}
              </option>
            </select>
          </template>

          <!-- Affichage de la table déduite depuis une réservation -->
          <span
            v-if="formulaire.idtable && formulaire.idreservSelectionnee !== 'nouveau'"
            class="table-deduite"
          >
    ...
    <table v-else class="tableau-commandes">
      ...
      <tbody>
        <tr v-for="cmd in commandes" :key="cmd.idcom">
          <td>{{ cmd.idcom.toUpperCase() }}</td>
          <td>{{ cmd.nomcli ? cmd.nomcli.toUpperCase() : 'Non spécifié' }}</td>
          ...
          <td>
            ...
            <button @click="ouvrirAddition(cmd.idcom)">Addition PDF</button>
    ...
    >
      <p><strong>Client :</strong> {{ commandeDetail.nomcli ? commandeDetail.nomcli.toUpperCase() : 'Non spécifié' }}</p>
      <p>
        <strong>Type :</strong>
